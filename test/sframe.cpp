...
TEST_CASE("SFrame Context Remove Key")
{
  ...
  SFRAME_VALUE_OR_THROW(sender.add_key(kid, KeyUsage::protect, key));
  SFRAME_VALUE_OR_THROW(receiver.add_key(kid, KeyUsage::unprotect, key));

  // Protect and unprotect succeed before removal
  auto encrypted =
    to_bytes(SFRAME_VALUE_OR_THROW(sender.protect(kid, ct_out, plaintext, metadata)));
  auto decrypted =
    to_bytes(SFRAME_VALUE_OR_THROW(receiver.unprotect(pt_out, encrypted, metadata)));
  CHECK(decrypted == plaintext);

  // Remove sender key and verify protect fails
  sender.remove_key(kid);
  CHECK(
    sender.protect(kid, ct_out, plaintext, metadata)
      .error().type() == SFrameErrorType::invalid_parameter_error);

  // Remove receiver key and verify unprotect fails
  receiver.remove_key(kid);
  CHECK(
    receiver.unprotect(pt_out, encrypted, metadata)
      .error().type() == SFrameErrorType::invalid_parameter_error);
...
TEST_CASE("MLS Remove Epoch")
{
  ...
  auto enc = SFRAME_VALUE_OR_THROW(
    member_a.protect(epoch_id_1, sender_id, ct_out, plaintext, metadata));
  auto enc_data = to_bytes(enc);
  auto dec = to_bytes(SFRAME_VALUE_OR_THROW(
    member_b.unprotect(pt_out, enc_data, metadata)));
  CHECK(plaintext == dec);
  ...
  enc = SFRAME_VALUE_OR_THROW(
    member_a.protect(epoch_id_2, sender_id, ct_out, plaintext, metadata));
  dec = to_bytes(SFRAME_VALUE_OR_THROW(
    member_b.unprotect(pt_out, enc, metadata)));
  CHECK(plaintext == dec);
